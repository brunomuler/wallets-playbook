...
export default function YieldBearingAssetsDirectory() {
  // Handle both wrapper format { data: [...] } and direct array format
  const allAssets = (assetsData.data || assetsData) || [];
  
  // Filter to only yield-bearing assets
  const assets = allAssets.filter(asset => asset.yield_bearing === true);
  
  // Handle empty or malformed data
  if (!Array.isArray(assets) || assets.length === 0) {
    return (
      <div className={styles.gridContainer}>
        <p>No yield-bearing assets data available.</p>
      </div>
    );
  }
